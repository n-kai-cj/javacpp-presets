...
public class ImShowThread implements Runnable {
    ...
    public void run() {
        ...
        while (this.loopFlag) {
            ...
            if (!isLaunching &&
                ...
                isLaunching = true;
                try {
                    Platform.startup(() -> launchFxWindow(canvas, cls.width, cls.height, this.window_name));
                } catch (IllegalStateException ise) {
                    System.out.println(String.format("warn %s:%s", ise.getClass().getSimpleName(), ise.getMessage()));
                    launchFxWindow(canvas, cls.width, cls.height, this.window_name);
                }
                width = cls.width;
                height = cls.height;
    ...
    private void launchFxWindow(Canvas c, int w, int h, String name) {
        Platform.runLater(() -> {
            StackPane root = new StackPane();
            AnchorPane anchorPane = createPaneForCanvas(c);
    ...
    private AnchorPane createPaneForCanvas(Canvas canvas) {
