...
public class WaveHeader {
    ...
    public void write(File pcmFile, OutputStream out) throws IOException {
        byte[] pcm = new byte[100 * 1024];
        write(out);
        BufferedInputStream bis = new BufferedInputStream(new FileInputStream(pcmFile));
        int size = bis.read(pcm, 0, pcm.length);
        while (size > 0) {
            out.write(pcm, 0, size);
            size = bis.read(pcm, 0, pcm.length);
        }
    }
